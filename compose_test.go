...
func TestTheCompanionKeepsItsDistanceFromTheEdge(t *testing.T) {
	...
	for _, tc := range []struct{ w, wantRight int }{{124, 5}, {80, 4}, {40, 3}, {30, 2}} {
		...
		// pacing shipped; before that the two were the same column. The balloon
		// used to be checked here too; it is no longer a property of the layout,
		// because it hangs off the companion's HEAD -- see TestTheBalloonPointsAtTheCompanion.
		if lay.SandTo != lay.CatX-1-lay.PaceSpan {
			t.Errorf("w=%d: the sand (to %d) is not anchored to the companion's reach: it stands at %d and paces %d inward",
				tc.w, lay.SandTo, lay.CatX, lay.PaceSpan)
		}
		if lay.PaceSpan < 1 {
